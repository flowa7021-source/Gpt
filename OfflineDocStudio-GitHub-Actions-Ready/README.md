...
The repository now includes `.github/workflows/windows-installer.yml`.

How to use it:

1. Upload this project to a GitHub repository.
2. Open **Actions** in GitHub.
3. Run the **windows-installer** workflow manually via **Run workflow**, or push to `main` / `master`.
4. After the run completes, open the workflow run and download the artifact **OfflineDocStudio-windows-Release**.
5. The artifact contains the generated `Setup.exe` from `dist/`.

To publish installer files as GitHub Release assets too, create a GitHub Release and publish it. The workflow will attach files from `dist/` to that release automatically.
